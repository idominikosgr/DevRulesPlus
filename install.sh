#!/bin/sh
# AIDevRules Installer
# A universal shell script to install AIDevRules into any project with interactive setup
# Usage: curl -fsSL https://raw.githubusercontent.com/yourusername/AIDevRules/main/install.sh | sh

set -e

# Default values
REPO_URL="https://github.com/yourusername/AIDevRules"
TEMP_DIR="/tmp/aidevrules-$(date +%s)"
BRANCH="main"
METHOD="git"
UPGRADE=false
RUN_WIZARD=true
...
if [ "$RUN_WIZARD" = true ]; then
  ...
  if [ -f "$TEMP_DIR/setup-wizard.js" ]; then
    ...
    if [ $WIZARD_EXIT -ne 0 ]; then
      ...
      echo "✅ AIDevRules installed successfully via setup wizard"
    fi
  else
...
if [ "$RUN_WIZARD" = false ]; then
  echo "📋 Installing AIDevRules directly to $TARGET_DIR..."
  ...
  echo "✅ AIDevRules installed successfully to $TARGET_DIR"
fi
...
echo "curl -fsSL https://raw.githubusercontent.com/yourusername/AIDevRules/main/install.sh | sh -s -- --upgrade"
echo "=====================================" 